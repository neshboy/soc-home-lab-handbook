...
const REPO_ROOT = "C:/Users/User/projects/soc-home-lab-handbook";
...
// "### Section X — Name" subheadings — same shape as the Detection Engineering Handbook's
// and SOC Manager's Operating Handbook's indexes.
function parseIndex() {
  ...
  for (const raw of lines) {
    ...
    if (line === "## Appendix Table") {
      // The 22 parts are drafted/reviewed for this release; the 5 appendices listed in
      // BOOK-INDEX.md have not been authored yet (appendices/ is empty) — stop here rather
      // than emitting MISSING FILE placeholders for content that doesn't exist yet.
      break;
    }
...
// This book's six content tags are organized by build phase (BOOK-INDEX.md provenance item 1),
// not by professional role like the sibling volumes — different tag set, same colorizing mechanism.
const TAG_COLORS = {
  CONCEPT: "tag-concept",
  SETUP: "tag-setup",
  "HANDS-ON LAB": "tag-handson",
  TROUBLESHOOTING: "tag-trouble",
  SAFETY: "tag-safety",
  "COST/RESOURCE": "tag-cost",
};
...
function main() {
  ...
  :root {
    ...
    --magenta: #e87ba4; --green: #008300; --violet: #4a3aa7; --red: #d1382f;
    --ink: #0b0b0b; --ink-secondary: #52514e; --ink-muted: #898781;
    --gridline: #e1e0d9; --baseline: #c3c2b7;
    --blue-tint: #cde2fb; --orange-tint: #f9d9c4; --aqua-tint: #bdeedb; --yellow-tint: #fce8bf; --violet-tint: #e3ddf7; --magenta-tint: #fadce8; --green-tint: #cdeccd; --red-tint: #f7d3d0;
  }
  ...
  * { box-sizing: border-box; }
  ...
  .tag-setup { background: var(--blue-tint); color: #164a8a; border: 0.5pt solid var(--blue); }
  .tag-handson { background: var(--aqua-tint); color: #0d6b49; border: 0.5pt solid var(--aqua); }
  .tag-trouble { background: var(--orange-tint); color: #93401d; border: 0.5pt solid var(--orange); }
  .tag-safety { background: var(--red-tint); color: #8a2119; border: 0.5pt solid var(--red); }
  .tag-cost { background: var(--yellow-tint); color: #7a5400; border: 0.5pt solid var(--yellow); }
  `;
...
<head>
<meta charset="utf-8">
<title>The SOC Home Lab Handbook</title>
<style>${css}</style>
</head>
<body>
<section class="cover">
  <div class="kicker">NESHBOY SOC Professional Library</div>
  <h1>THE SOC HOME LAB HANDBOOK</h1>
  <div class="accent-rule"></div>
  <h2>Building a Personally-Operated, Isolated Practice Lab — SIEM, Endpoints, Network Monitoring, Honeypots, and Safe Attack Simulation</h2>
