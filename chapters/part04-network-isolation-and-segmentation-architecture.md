...
This part adds one refinement to Part 2's three-network vocabulary rather than contradicting it: the network-security-monitoring sensor built in Parts 12 through 14 gets its own broadcast domain for its packet-capture interface, because a mirror/SPAN port carries a full copy of lab/victim traffic and shouldn't need — or get — a routable address anywhere. Its separate log-shipping interface lives on that same NSM segment and reaches the SIEM through one explicit, narrowly-scoped allow rule — not a free pass just because both ends are "admin-facing." That gives this part four VLANs to design against instead of Part 2's three names, with no conflict between them.
...
| 20 | Lab/victim | Windows/Linux endpoints under test (Parts 9-10); attack-simulation targets (Part 18) | Management VLAN only, for admin SSH/WinRM | SIEM ingest ports only (Part 8) |
...
| VLAN 40 (Honeynet) | SIEM host (VLAN 10) | `5044`/tcp (log forwarding) | Allow |
| VLAN 10 (Management) | VLAN 20 (Lab/victim) | `22`/tcp, `5985`/tcp, `5986`/tcp (admin SSH/WinRM) | Allow |
| VLAN 10 (Management) | VLAN 30 (NSM) | `22`/tcp | Allow |
| VLAN 10 (Management) | VLAN 40 (Honeynet) | `22`/tcp, from a single jump-host address only | Allow |
| VLAN 20 (Lab/victim) | VLAN 10 (Management) | any | Deny |
| VLAN 20 (Lab/victim) | SIEM host (VLAN 10) | `1514`/tcp, `1515`/tcp | Allow |
| VLAN 30 (NSM) | SIEM host (VLAN 10) | `5044`/tcp (log forwarding) | Allow |
| VLAN 30 (NSM) | Internet | any | Deny |
| Admin workstation | VLAN 10 (Management) | `443`/tcp, `22`/tcp | Allow |
| any | any | any | Deny |

Two rows deserve a second look before you treat this table as complete. First, the admin SSH/WinRM row exists because Table 4.1 promises the lab/victim VLAN accepts inbound admin access from the management VLAN — without an explicit allow rule for it, the terminal deny-all row below would silently block the exact access path Table 4.1 says exists, and you'd spend an afternoon debugging a "broken" WinRM connection that the rule set never actually permitted. Second, the `VLAN 30 (NSM) → SIEM host` row is not the sensor's mirror/SPAN capture interface talking — Section 1 established that interface gets no routable address at all — it's the sensor's separate management-path interface, which sits on the NSM VLAN itself rather than on VLAN 10, reaching the SIEM through this one explicit, narrowly-scoped allow rule instead of an implicit, unrouted "it's already on the management segment" pass. The `VLAN 30 (NSM) → Internet: Deny` row above stays anyway, as a belt-and-suspenders rule limiting that same interface to the one destination it actually needs.
